...
public class Paillier {
	...
	public static BigInteger encrypt(BigInteger m, BigInteger r, BigInteger g, BigInteger n, BigInteger nsquare) {
		...
		return BigIntegerUtils.mulMod(BigIntegerUtils.modPow(g, m, nsquare), BigIntegerUtils.modPow(r, n, nsquare), nsquare);
//        return (BigIntegerUtils.modPow(g, m, nsquare).multiply(
//                BigIntegerUtils.modPow(r, n, nsquare)).mod(nsquare));
	}
...
//        BigIntegerUtils.modPow(c,prikey.getLambda(),prikey.getNsquare());
		BigInteger c1 = BigIntegerUtils.modPow(c, prikey.getLambda(), prikey.getNsquare());

		//after we calculate c1=c^d mod n^2 = (1+mn*lambda) mod n^2
		// we now find (c1-1)/n=m*lambda mod n
		//therefore m= lambda^-1*(c1-1)/n mod n
		BigInteger m = BigIntegerUtils.mulMod(c1.subtract(BigInteger.ONE).divide(prikey.getN()), prikey.getLmdInvs(), prikey.getN());
//        BigInteger m = c1.subtract(BigInteger.ONE).divide(prikey.getN()).multiply(prikey.getLmdInvs()).mod(prikey.getN());
	...
	public BigInteger add(BigInteger c1, BigInteger c2) {
		...
		return BigIntegerUtils.mulMod(c1, c2, pubkey.getNsquare());
//        return (c1.multiply(c2)).mod(pubkey.getNsquare());
	...
	public BigInteger multiply(BigInteger c, BigInteger cons) {
		...
		return BigIntegerUtils.modPow(c, cons, pubkey.getNsquare());
