...
public final class Gmp implements AutoCloseable {
    ...
    public static BigInteger MulInverse(BigInteger val, BigInteger modulus) {
        return INSTANCE.get().MulImpl(val, modulus);
    }

    public static BigInteger mulMod(BigInteger val, BigInteger b, BigInteger modulus) {
        return INSTANCE.get().mulModImpl(val, b, modulus);
    }
    ...
    private BigInteger MulImpl(BigInteger dividend, BigInteger divisor) {
        LibGmp.mpz_t dividendPeer = getPeer(dividend, sharedOperands[0]);
        LibGmp.mpz_t divisorPeer = getPeer(divisor, sharedOperands[1]);

        LibGmp.__gmpz_mul(sharedOperands[2], dividendPeer, divisorPeer);
        // The result size is never larger than the bit length of the dividend minus that of the divisor
        // plus 1 (but is at least 1 bit long to hold the case that the two values are exactly equal)
        int requiredSize = dividend.bitLength() + 1;
        return new BigInteger(mpzSgn(sharedOperands[2]), mpzExport(sharedOperands[2], requiredSize));
    }

    private BigInteger mulModImpl(BigInteger dividend, BigInteger divisor, BigInteger mod) {
        LibGmp.mpz_t dividendPeer = getPeer(dividend, sharedOperands[0]);
        LibGmp.mpz_t divisorPeer = getPeer(divisor, sharedOperands[1]);
        LibGmp.mpz_t modPeer = getPeer(mod, sharedOperands[3]);

        LibGmp.__gmpz_mul(sharedOperands[2], dividendPeer, divisorPeer);
        LibGmp.__gmpz_mod(sharedOperands[1], sharedOperands[2], modPeer);
        // The result size is never larger than the bit length of the dividend minus that of the divisor
        // plus 1 (but is at least 1 bit long to hold the case that the two values are exactly equal)
        int requiredSize = mod.bitLength();
